...
class Scanner:
	...
	def get_token(self):
		...
		# if there are extra non-white-space characters
		if token is None and self.current_char_index < len(self.input_string) - 1:
			self.no_token()
...
class If_AST:
	...
	def __init__(self, bool_expression, then):
		self.bool_expression = bool_expression
		self.then = then

	def __repr__(self):
		return 'if ' + repr(self.bool_expression) + ' then ' + \
					   repr(self.then) + ' end'

	def indented(self, level):
		return indent('If', level) + \
			   self.bool_expression.indented(level+1) + \
			   self.then.indented(level+1)

	def code(self):
		l1 = label_generator.next()
		return self.bool_expression.false_code(l1) + \
			   self.then.code() + \
			   l1 + ':\n'
...
class If_Else_AST:
	...
	def __init__(self, bool_expression, then, otherwise):
		self.bool_expression = bool_expression
		self.then = then
...
class While_AST:
	...
	def __init__(self, bool_term, body):
		self.bool_term = bool_term
		self.body = body

	def __repr__(self):
		return 'while ' + repr(self.bool_term) + ' do ' + \
						  repr(self.body) + ' end'

	def indented(self, level):
		return indent('While', level) + \
			   self.bool_term.indented(level+1) + \
...
def bool_expression():
	result = bool_term()
	while scanner.lookahead() is Token.OR:
...
def bool_term():
	result = bool_factor()
	while scanner.lookahead() is Token.AND:
...
def bool_factor():
	if scanner.lookahead() is Token.NOT:
		op = scanner.consume(Token.NOT)
		tree = bool_factor()
		result = Boolean_AST(tree, op)
	else:
		result = comparison()
